...
// httpryFormula represents a formula in Go.
type httpryFormula struct {
	Description  string
	Homepage     string
	URL          string
	Sha256       string
...
func (pkg httpryFormula) Print() {
	fmt.Printf("Name: httpry\n")
...
func main() {
	...
	if err := pkg.Installhttpry(); err != nil {
...
func (pkg httpryFormula) Installhttpry() error {
	cmd := exec.Command("curl", "-O", pkg.URL)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to download: %v", err)
	}

	tarball := "httpry-0.1.8.tar.gz"
	cmd = exec.Command("tar", "-xf", tarball)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to extract tarball: %v", err)
	}

	sourceDir := "httpry-0.1.8.tar"
