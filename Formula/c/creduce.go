...
// creduceFormula represents a formula in Go.
type creduceFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func main() {
    pkg := creduceFormula{
        ...
        Dependencies: []string{"astyle", "llvm"},
    }
    ...
    if err := pkg.Installcreduce(); err != nil {
...
func (pkg creduceFormula) Installcreduce() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "URI-1.72.tar.gz"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "URI-1.72.tar"
