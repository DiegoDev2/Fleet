...
// cgitFormula represents a formula in Go.
type cgitFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg cgitFormula) Print() {
    fmt.Printf("Name: cgit\n")
...
func main() {
    pkg := cgitFormula{
        ...
        Dependencies: []string{"gettext", "openssl@3"},
    }
    ...
    if err := pkg.Installcgit(); err != nil {
...
func (pkg cgitFormula) Installcgit() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "memrchr-impl.patch"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "memrchr-impl"
