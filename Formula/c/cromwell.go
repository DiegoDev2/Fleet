...
// cromwellFormula represents a formula in Go.
type cromwellFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg cromwellFormula) Print() {
...
func main() {
    ...
    if err := pkg.Installcromwell(); err != nil {
...
func (pkg cromwellFormula) Installcromwell() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "womtool-87.jar"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "womtool-87"
