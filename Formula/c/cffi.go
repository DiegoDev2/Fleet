...
// cffiFormula represents a formula in Go.
type cffiFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func main() {
    pkg := cffiFormula{
        ...
        Dependencies: []string{"python@3.11", "python@3.12", "pycparser"},
    }
    ...
    if err := pkg.Installcffi(); err != nil {
...
func (pkg cffiFormula) Installcffi() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "cffi-1.17.0.tar.gz"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "cffi-1.17.0.tar"
