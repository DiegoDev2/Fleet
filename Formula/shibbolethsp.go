...
func installShibbolethSp() {
	...
	exec.Command("latte", "install", "log4shib").Run()
	fmt.Println("Instalando dependencia: opensaml")
	exec.Command("latte", "install", "opensaml").Run()
	...
	exec.Command("latte", "install", "xml-security-c").Run()
	fmt.Println("Instalando dependencia: xml-tooling-c")
	exec.Command("latte", "install", "xml-tooling-c").Run()
}
