...
// erdtreeFormula represents a formula in Go.
type erdtreeFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
", dep)
    ...
    if err := pkg.Installerdtree(); err != nil {
...
func (pkg erdtreeFormula) Installerdtree() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "v3.1.2.tar.gz"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "v3.1.2.tar"
