...
// plenvFormula represents a formula in Go.
type plenvFormula struct {
	Description  string
	Homepage     string
	URL          string
	Sha256       string
...
func main() {
	pkg := plenvFormula{
		...
		Dependencies: []string{"perl-build"},
	}
	...
	if err := pkg.Installplenv(); err != nil {
...
func (pkg plenvFormula) Installplenv() error {
	cmd := exec.Command("curl", "-O", pkg.URL)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to download: %v", err)
	}

	tarball := "2.3.1.tar.gz"
