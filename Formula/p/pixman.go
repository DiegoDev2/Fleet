...
// pixmanFormula represents a formula in Go.
type pixmanFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func main() {
    ...
    if err := pkg.Installpixman(); err != nil {
...
func (pkg pixmanFormula) Installpixman() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "pixman-0.42.2.patch"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "pixman-0.42.2"
