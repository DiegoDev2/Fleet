...
// pstreeFormula represents a formula in Go.
type pstreeFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func main() {
    ...
    if err := pkg.Installpstree(); err != nil {
...
func (pkg pstreeFormula) Installpstree() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "v2.40.tar.gz"
