...
// argon2Formula represents a formula in Go.
type argon2Formula struct {
	Description  string
	Homepage     string
	URL          string
	Sha256       string
...
func main() {
	...
	if err := pkg.Installargon2(); err != nil {
...
func (pkg argon2Formula) Installargon2() error {
	cmd := exec.Command("curl", "-O", pkg.URL)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to download: %v", err)
	}

	tarball := "20190702.tar.gz"
	cmd = exec.Command("tar", "-xf", tarball)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to extract tarball: %v", err)
	}

	sourceDir := "20190702.tar"
