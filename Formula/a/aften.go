...
// aftenFormula represents a formula in Go.
type aftenFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
", dep)
    ...
    if err := pkg.Installaften(); err != nil {
...
func (pkg aftenFormula) Installaften() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "1kHz_44100Hz_16bit_05sec.wav"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "1kHz_44100Hz_16bit_05sec"
