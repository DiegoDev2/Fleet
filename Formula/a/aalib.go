...
// aalibFormula represents a formula in Go.
type aalibFormula struct {
	Description  string
	Homepage     string
	URL          string
	Sha256       string
...
func main() {
	...
	if err := pkg.Installaalib(); err != nil {
...
func (pkg aalibFormula) Installaalib() error {
	cmd := exec.Command("curl", "-O", pkg.URL)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to download: %v", err)
	}

	tarball := "1.4rc5.patch"
