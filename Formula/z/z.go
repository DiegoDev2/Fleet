...
// zFormula represents a formula in Go.
type zFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg zFormula) Installz() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "v1.12.tar.gz"
