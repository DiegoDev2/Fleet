...
// ghiFormula represents a formula in Go.
type ghiFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg ghiFormula) Installghi() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "pygments.rb-2.3.0.gem"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "pygments.rb-2.3.0"
