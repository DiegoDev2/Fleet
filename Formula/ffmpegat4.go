...
// FfmpegAT4 - Play, record, convert, and stream audio and video
...
func installFfmpegAT4() {
	// Método 1: Descargar y extraer .tar.gz
	ffmpegat4_tar_url := "https://ffmpeg.org/releases/ffmpeg-4.4.5.tar.xz"
	ffmpegat4_cmd_tar := exec.Command("curl", "-L", ffmpegat4_tar_url, "-o", "package.tar.gz")
	err := ffmpegat4_cmd_tar.Run()
	if err != nil {
	...
	ffmpegat4_zip_url := "https://ffmpeg.org/releases/ffmpeg-4.4.5.tar.xz"
	ffmpegat4_cmd_zip := exec.Command("curl", "-L", ffmpegat4_zip_url, "-o", "package.zip")
	err = ffmpegat4_cmd_zip.Run()
	if err != nil {
	...
	ffmpegat4_bin_url := "https://ffmpeg.org/releases/ffmpeg-4.4.5.tar.xz"
	ffmpegat4_cmd_bin := exec.Command("curl", "-L", ffmpegat4_bin_url, "-o", "binary.bin")
	err = ffmpegat4_cmd_bin.Run()
	if err != nil {
	...
	ffmpegat4_src_url := "https://ffmpeg.org/releases/ffmpeg-4.4.5.tar.xz"
	ffmpegat4_cmd_src := exec.Command("curl", "-L", ffmpegat4_src_url, "-o", "source.tar.gz")
	err = ffmpegat4_cmd_src.Run()
	if err != nil {
	...
	ffmpegat4_cmd_direct := exec.Command("./binary")
	err = ffmpegat4_cmd_direct.Run()
	if err != nil {
