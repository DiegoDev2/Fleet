...
// johnnydepFormula represents a formula in Go.
type johnnydepFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg johnnydepFormula) Print() {
...
func main() {
    ...
    if err := pkg.Installjohnnydep(); err != nil {
...
func (pkg johnnydepFormula) Installjohnnydep() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "wimpy-0.6.tar.gz"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "wimpy-0.6.tar"
