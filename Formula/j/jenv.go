...
// jenvFormula represents a formula in Go.
type jenvFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
", dep)
    ...
    if err := pkg.Installjenv(); err != nil {
...
func (pkg jenvFormula) Installjenv() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "0.5.7.tar.gz"
