...
// blackboxFormula represents a formula in Go.
type blackboxFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg blackboxFormula) Print() {
...
", dep)
    ...
    if err := pkg.Installblackbox(); err != nil {
...
func (pkg blackboxFormula) Installblackbox() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "v1.20220610.tar.gz"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "v1.20220610.tar"
