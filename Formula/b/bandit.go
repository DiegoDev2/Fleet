...
// banditFormula represents a formula in Go.
type banditFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
", dep)
    ...
    if err := pkg.Installbandit(); err != nil {
...
func (pkg banditFormula) Installbandit() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "stevedore-5.2.0.tar.gz"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "stevedore-5.2.0.tar"
