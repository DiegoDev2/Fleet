...
// braidFormula represents a formula in Go.
type braidFormula struct {
	Description  string
	Homepage     string
	URL          string
	Sha256       string
...
func main() {
	...
	if err := pkg.Installbraid(); err != nil {
...
func (pkg braidFormula) Installbraid() error {
	cmd := exec.Command("curl", "-O", pkg.URL)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to download: %v", err)
	}

	tarball := "map-6.6.0.gem"
