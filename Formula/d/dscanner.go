...
// dscannerFormula represents a formula in Go.
type dscannerFormula struct {
	Description  string
	Homepage     string
	URL          string
	Sha256       string
...
func (pkg dscannerFormula) Print() {
...
func main() {
	...
	if err := pkg.Installdscanner(); err != nil {
...
func (pkg dscannerFormula) Installdscanner() error {
	cmd := exec.Command("curl", "-O", pkg.URL)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to download: %v", err)
	}

	tarball := "D-Scanner.git"
