...
// darkstatFormula represents a formula in Go.
type darkstatFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg darkstatFormula) Print() {
...
", dep)
    ...
    if err := pkg.Installdarkstat(); err != nil {
...
func (pkg darkstatFormula) Installdarkstat() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "clock_gettime.patch"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "clock_gettime"
