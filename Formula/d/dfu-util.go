...
// dfu-utilFormula represents a formula in Go.
type dfu-utilFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg dfu-utilFormula) Print() {
    fmt.Printf("Name: dfu-util\n")
...
func main() {
    ...
    if err := pkg.Installdfu-util(); err != nil {
...
func (pkg dfu-utilFormula) Installdfu-util() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "dfu-util.git"
