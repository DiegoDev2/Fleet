...
// rlwrapFormula represents a formula in Go.
type rlwrapFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func main() {
    ...
    if err := pkg.Installrlwrap(); err != nil {
...
func (pkg rlwrapFormula) Installrlwrap() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "0.46.1.tar.gz"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "0.46.1.tar"
