...
// okta-awscliFormula represents a formula in Go.
type okta-awscliFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg okta-awscliFormula) Print() {
    fmt.Printf("Name: okta-awscli\n")
...
func main() {
    ...
    if err := pkg.Installokta-awscli(); err != nil {
...
func (pkg okta-awscliFormula) Installokta-awscli() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "validators-0.28.3.tar.gz"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "validators-0.28.3.tar"
