...
// mcapFormula represents a formula in Go.
type mcapFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg mcapFormula) Print() {
    fmt.Printf("Name: mcap\n")
...
func main() {
    ...
    if err := pkg.Installmcap(); err != nil {
...
func (pkg mcapFormula) Installmcap() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "OneMetadata-mdx-pad-st-sum.mcap"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "OneMetadata-mdx-pad-st-sum"
