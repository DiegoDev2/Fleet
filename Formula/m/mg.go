...
// mgFormula represents a formula in Go.
type mgFormula struct {
	Description  string
	Homepage     string
	URL          string
	Sha256       string
...
func (pkg mgFormula) Installmg() error {
	cmd := exec.Command("curl", "-O", pkg.URL)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to download: %v", err)
	}

	tarball := "mg-7.3.tar.gz"
	cmd = exec.Command("tar", "-xf", tarball)
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("failed to extract tarball: %v", err)
	}

	sourceDir := "mg-7.3.tar"
