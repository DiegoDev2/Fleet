...
// ncpFormula represents a formula in Go.
type ncpFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg ncpFormula) Installncp() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "GNUmakefile.diff"
    cmd = exec.Command("tar", "-xf", tarball)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to extract tarball: %v", err)
    }

    sourceDir := "GNUmakefile"
