...
// xclogparserFormula represents a formula in Go.
type xclogparserFormula struct {
    Description  string
    Homepage     string
    URL          string
    Sha256       string
...
func (pkg xclogparserFormula) Print() {
...
func main() {
    ...
    if err := pkg.Installxclogparser(); err != nil {
...
func (pkg xclogparserFormula) Installxclogparser() error {
    cmd := exec.Command("curl", "-O", pkg.URL)
    if err := cmd.Run(); err != nil {
        return fmt.Errorf("failed to download: %v", err)
    }

    tarball := "test.xcactivitylog"
